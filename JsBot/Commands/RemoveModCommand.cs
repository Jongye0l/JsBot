...
public class RemoveModCommand : ApplicationCommandModule<ApplicationCommandContext> {
	[SlashCommand("removemod", "모드를 제거합니다.")]
	public async Task Handle([SlashCommandParameter(Description = "모드 이름", AutocompleteProviderType = typeof(AllJModNameAutocompleteProvider))] string name) {
		...
		await mod.Delete();
		new LogBuilder(Context.User, "모드를 제거했습니다")
			...
			.Send();
		await RespondAsync(InteractionCallback.Message(new InteractionMessageProperties().WithContent(name + " 모드를 제거했습니다.").WithFlags(MessageFlags.Ephemeral)));
	}
}